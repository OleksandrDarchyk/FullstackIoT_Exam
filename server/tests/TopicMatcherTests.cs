...
public class TopicMatcherTests
{
    public class MatchesTests
    {
        ...
        [Fact]
        public void MultiLevelWildcard_MatchesTopicWithoutTrailingSegment()
        {
            // Per MQTT spec, "devices/#" must also match bare "devices" (no slash after prefix).
            // Some implementations incorrectly require at least one segment after the slash.
            Assert.True(TopicMatcher.Matches("devices/#", "devices"));
        }

        [Fact]
        public void SingleLevelWildcard_DoesNotMatchBarePrefix()
        {
            // "+" requires exactly one segment — "devices/+" must NOT match "devices".
            Assert.False(TopicMatcher.Matches("devices/+", "devices"));
        }

        [Fact]
        public void HashNotLastInPattern_TreatedAsTerminal()
        {
            // MQTT spec treats "#" as terminal (everything after it in the pattern is ignored).
            // This library is lenient: "devices/#/x" behaves identically to "devices/#".
            Assert.True(TopicMatcher.Matches("devices/#/x", "devices/sensor1"));
            Assert.True(TopicMatcher.Matches("devices/#/x", "devices/sensor1/telemetry"));
            Assert.False(TopicMatcher.Matches("devices/#/x", "other/sensor1"));
        }
    }
